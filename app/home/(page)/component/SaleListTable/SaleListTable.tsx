...
const SaleListTable: FC = () => {
    ...
    const [searchCardCode, setSearchCardCode] = useState<string>('');
    ...
    const filteredProduct = dataSale.filter((p) => {
        return (
            (!searchUUID || p.uuid.toLowerCase().includes(searchUUID.toLowerCase())) &&
            (!searchFirstName || p.first_name.toLowerCase().includes(searchFirstName.toLowerCase())) &&
            (!searchLastName || p.last_name.toLowerCase().includes(searchLastName.toLowerCase())) &&
            (!searchPhone || p.phone.toLowerCase().includes(searchPhone.toLowerCase())) &&
            (!searchCardCode || p.id_card_code.toLowerCase().includes(searchCardCode.toLowerCase())) 
        );
    });

    // กรองค่าที่ซ้ำออกจาก data ที่จะส่งให้ Autocomplete
    const uniqueUUID = Array.from(new Set(filteredProduct.map(product => product.uuid)));
    const uniqueFirstName = Array.from(new Set(filteredProduct.map(product => product.first_name)));
    const uniqueLastName = Array.from(new Set(filteredProduct.map(product => product.last_name)));
    const uniquesPhone = Array.from(new Set(filteredProduct.map(product => product.phone)));
    const uniquesCardCode = Array.from(new Set(filteredProduct.map(product => product.id_card_code)));

    const autocompleteData = [
        { value: searchUUID, setValue: setSearchUUID, data: uniqueUUID, placeholder: "Search UUID" },
        { value: searchFirstName, setValue: setSearchFirstName, data: uniqueFirstName, placeholder: "Search Firstname" },
        { value: searchLastName, setValue: setSearchLastName, data: uniqueLastName, placeholder: "Search Lastname" },
        { value: searchPhone, setValue: setSearchPhone, data: uniquesPhone, placeholder: "Search Phone" },
        { value: searchCardCode, setValue: setSearchCardCode, data: uniquesCardCode, placeholder: "Search ID Card Code" },
    ];
    ...
    const rows = filteredProduct.map((row, index) => {
        ...
        return (
            ...
            >
                ...
                <Table.Td>
                    <Center>
                        <Text size={'sm'} c='var(--mantine-color-gray-8)' ta={'center'}>{row.uuid}</Text>
                    </Center>
                </Table.Td>
                <Table.Td>
                    <Center>
                        <Text size={'sm'} c='var(--mantine-color-gray-8)' ta={'center'}>{row.first_name}</Text>
                    </Center>
                </Table.Td>
                <Table.Td>
                    <Center>
                        <Text size={'sm'} c='var(--mantine-color-gray-8)' ta={'start'}>{row.last_name}</Text>
                    </Center>
                </Table.Td>
                <Table.Td>
                    <Center>
                ...
                <Table.Td>
                    <Center>
                        <Text size={'sm'} c='var(--mantine-color-gray-8)' ta={'start'}>{row.id_card_code}</Text>
                    </Center>
                </Table.Td>
            </Table.Tr >
        );
            ...
            <Table verticalSpacing="xs">
                <Table.Thead bg='var(--mantine-color-gray-1)'>
                    <Table.Tr>
                        ...
                        <Table.Th><Center>{columnName('UUID')}</Center></Table.Th>
                        <Table.Th><Center>{columnName('First name')}</Center></Table.Th>
                        <Table.Th><Center>{columnName('Last name')}</Center></Table.Th>
                        <Table.Th><Center>{columnName('Phone')}</Center></Table.Th>
                        <Table.Th><Center>{columnName('ID Card Code')}</Center></Table.Th>
                    </Table.Tr>
                </Table.Thead>
